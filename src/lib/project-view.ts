...
import { complianceScore, computeStatus, effectiveStatusSettings, StatusKey, statusMeta, worstStatus } from "@/lib/status";
...
export function statusForClient(client: Client, types: ProjectType[], records: ProjectRecord[], settings: Settings) {
  ...
  const statuses = types.map((type) => computeStatus(byType.get(type.id) ?? { year: null, requested: false }, effectiveStatusSettings(settings, type)));
